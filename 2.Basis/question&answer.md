...
- #### $L_1-Regularization$ 和$L_2-Regularization$的区别
  ...
  加入l1正则化的目的是为了加强权值的稀疏性，让更多值接近于零。而l2正则化则是为了减小每次权重的调整幅度，避免模型训练过程中出现较大抖动。 
  ...
  正则项是为了降低模型的复杂度，从而避免模型区过分拟合训练数据，包括噪声与异常点（outliers）。从另一个角度上来讲，正则化即是模型参数添加先验，这样就规定了参数的分布，使得模型的复杂度降低，这样模型对于噪声与异常点的抗干扰性的能力增强，从而提高模型的泛化能力。 在数据少的时候，先验知识可以防止过拟合。 
